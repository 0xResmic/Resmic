import "./main.css";
import {jsx as $erTRO$jsx, Fragment as $erTRO$Fragment, jsxs as $erTRO$jsxs} from "react/jsx-runtime";
...
import {Select as $erTRO$Select} from "antd";
import {Buffer as $erTRO$Buffer} from "buffer";
...
*/ const $0f9c8968340f6221$var$nativeTokenPayment = async (_amount, _address, _tokenSymbol, _paymentConfirmations)=>{
    ...
    tokenAmount = (0, $erTRO$bignumberjs)(Math.floor(parseFloat(tokenAmount) * 10 ** decimal));
   ...
   */ const $0f9c8968340f6221$var$requestERC20Payment = async (_amount, _tokenAddress, _userAddress, _toAddress, _paymentConfirmations)=>{
    ...
    try {
       ...
       */ const contractInstance = new (0, $erTRO$ethers).Contract(_tokenAddress, (0, $535c395a1d197c28$export$89843982d7e60b14), signer);
        ...
        amount = (0, $erTRO$bignumberjs)(Math.floor(parseFloat(amount) * 10 ** decimals));
 ...
 */ const $8719e4e81b688ff3$var$requestERC20Payment = async (_tokenAmount, _tokenAddress, _toAddress, _fromAddress, _provider, _signer, _blockConfirmations, _starknetObj)=>{
    ...
    let tokenAmount = (0, $erTRO$bignumberjs)(Math.floor(parseFloat(_tokenAmount) * 10 ** decimal));
    try {
...
async function $ababbd98e7557289$export$888627a074f2aa7d(Token, Address, Amount, PaymentConfirmation = 2) {
...
*/ const $0dd8249e93adf6d7$var$getProvider = ()=>{
    if ("solana" in window) {
...
async function $0dd8249e93adf6d7$export$43158100a9ac6874() {
    const provider = $0dd8249e93adf6d7$var$getProvider();
    if (provider) try {
    ...
    else (0, $erTRO$toast).error("Phantom wallet is not installed!", {
        position: (0, $erTRO$toast).POSITION.TOP_CENTER,
        theme: "dark"
    });
}
...
let $12556e0b190893f5$var$walletAddress = "";
async function $12556e0b190893f5$export$88ef2c5afd949ec(selectedToken, Address, Amount, noOfBlockConformation) {
    let receiverAddress = Address.SOLANA;
    let phantomWalletInstance = await (0, $0dd8249e93adf6d7$export$43158100a9ac6874)();
    ...
    $12556e0b190893f5$var$walletAddress = connectedWalletAddress;
    // Native transaction
    if (selectedToken === "SOL") {
        // Calculate the live token price.
        let liveTokenPrice = await (0, $34c3ac9406e6b976$export$6c774fdbdbcfd895)("solana");
        liveTokenPrice = parseFloat(liveTokenPrice);
        let calculatedTokenPrice = parseFloat(Amount) / liveTokenPrice;
        let tokenTransferStatus = $12556e0b190893f5$var$sendToken(receiverAddress, calculatedTokenPrice) // For native
        ;
        return tokenTransferStatus;
    } else {
        let tokenAddress = (0, $535c395a1d197c28$export$5c2c3f7af123bc40)["Solana"][selectedToken];
        let isTokenStable = (0, $535c395a1d197c28$export$8e1e81ac145e31be)[selectedToken].type;
        // Passing the amount as It is. As we are directly accpeting the stable coin to USD
        if (isTokenStable == "stable") {
            let tokenTransferStatus = $12556e0b190893f5$var$sendToken(receiverAddress, Amount, tokenAddress) // For SPL tokens
            ;
            return tokenTransferStatus;
        } else {
            let liveTokenPrice = await (0, $34c3ac9406e6b976$export$6c774fdbdbcfd895)("solana");
            liveTokenPrice = parseFloat(liveTokenPrice);
            let calculatedTokenPrice = parseFloat(Amount) / liveTokenPrice;
            let tokenTransferStatus = $12556e0b190893f5$var$sendToken(receiverAddress, calculatedTokenPrice, tokenAddress) // For SPL tokens
            ;
            return tokenTransferStatus;
        }
    }
 ...
 * @param {String} _tokenAddress 
 * @returns {Bool} Payment Status
 */ const $12556e0b190893f5$var$sendToken = async (_receiverAddress, _amount, _tokenAddress)=>{
    if (!$12556e0b190893f5$var$walletAddress) return alert("Connect your wallet first!");
    ...
    try {
        ...
        if (_tokenAddress) {
            ...
            const receiverTokenAddress = await (0, $erTRO$getAssociatedTokenAddress)(tokenPublicKey, receiverPublicKey);
            // SPL Token Transfer
            const senderTokenAddress = await (0, $erTRO$getAssociatedTokenAddress)(tokenPublicKey, senderPublicKey);
            const senderAccount = await (0, $erTRO$getAccount)(connection, senderTokenAddress);
            const mintInfo = await (0, $erTRO$getMint)(connection, tokenPublicKey);
            decimal = mintInfo.decimals;
            let _amountBigNumber = (0, $erTRO$bignumberjs)(Math.floor(parseFloat(_amount) * 10 ** decimal));
            if (senderAccount.amount <= _amountBigNumber) {
                (0, $erTRO$toast).error("Insufficient token balance.", {
                    position: (0, $erTRO$toast).POSITION.TOP_CENTER,
                    theme: "dark"
                });
                return false;
            }
            const transferInstruction = (0, $erTRO$createTransferInstruction)(senderTokenAddress, receiverTokenAddress, senderPublicKey, _amountBigNumber, [], (0, $erTRO$TOKEN_PROGRAM_ID));
            tx.add(transferInstruction);
        } else {
            // Native SOL Transfer
            let _calculatedAmount = (0, $erTRO$bignumberjs)(Math.floor(parseFloat(_amount) * 10 ** 9));
            const transferInstruction = (0, $erTRO$SystemProgram).transfer({
        ...
        if (_tokenAddress) {
            let amountCheckStatus = await $12556e0b190893f5$var$checkSPLTokenTransfer(connection, signature, _amount, decimal);
            return amountCheckStatus;
        } else {
            let amountCheckStatus = await $12556e0b190893f5$var$checkNativeTokenTransfer(connection, signature, _amount);
            return amountCheckStatus;
        }
    } catch (err) {
        console.error("Error sending transaction:", err);
        (0, $erTRO$toast).error("Transaction failed!", {
            position: (0, $erTRO$toast).POSITION.TOP_CENTER,
            theme: "dark"
        });
        return false;
...
// returns {Boolean}
const $12556e0b190893f5$var$checkSPLTokenTransfer = async (connection, signature, amount, tokenDecimals)=>{
    try {
        ...
        const transferInstruction = instructions.find((inst)=>inst.programId.equals((0, $erTRO$TOKEN_PROGRAM_ID)));
        if (transferInstruction) {
            ...
            if (parseFloat(transferredAmount) <= parseFloat((0, $erTRO$bignumberjs)(Math.floor(parseFloat(amount) * 10 ** tokenDecimals)))) {
                (0, $erTRO$toast).success("Payment done successfully:)", {
                    position: (0, $erTRO$toast).POSITION.TOP_CENTER,
                    theme: "dark"
                });
...
// returns {Boolean}
const $12556e0b190893f5$var$checkNativeTokenTransfer = async (connection, signature, amount)=>{
    try {
        const transactionDetails = await connection.getParsedTransaction(signature, "finalized");
        if (!transactionDetails) throw new Error("Transaction not found!");
        const instructions = transactionDetails.transaction.message.instructions;
        // Look for a transfer instruction from the SystemProgram
        const transferInstruction = instructions.find((inst)=>inst.programId.equals((0, $erTRO$SystemProgram).programId));
        if (transferInstruction) {
            const parsedData = transferInstruction.parsed.info;
            const transferredAmount = parsedData.lamports; // Convert lamports to SOL
            if (parseFloat(transferredAmount) <= parseFloat((0, $erTRO$bignumberjs)(Math.floor(parseFloat(amount) * 10 ** 9)))) {
...
window.Buffer = window.Buffer || (0, $erTRO$Buffer);
 ...
 */ function $79b4685724ad28b1$var$CryptoPayment({ Address: Address, Tokens: Tokens, Chains: Chains, Amount: Amount, noOfBlockConformation: noOfBlockConformation, setPaymentStatus: setPaymentStatus, Style: Style = {
     ...
     */ const makePayment = async ()=>{
        if (selectedBlockchain === "" || selectedToken === "") (0, $erTRO$toast).warning("Please select payment method", {
            ...
            case "Solana":
                setIsLoading(true);
                let makePaymentSolana = await (0, $12556e0b190893f5$export$88ef2c5afd949ec)(selectedToken, Address, Amount, noOfBlockConformation);
            ...
            case "Nibiru":
                setIsLoading(true);
                let makeNibiruPay = await (0, $ababbd98e7557289$export$888627a074f2aa7d)(selectedToken, Address, Amount, noOfBlockConformation);
    ...
    return /*#__PURE__*/ (0, $erTRO$jsx)((0, $erTRO$Fragment), {
        children: /*#__PURE__*/ (0, $erTRO$jsxs)("div", {
            className: "mainDivResmic",
            children: [
                                    ...
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0, $erTRO$jsxs)("div", {
                                            ...
                                            children: [
                                                ...
                                                /*#__PURE__*/ (0, $erTRO$jsx)((0, $erTRO$Select), {
                                                    placeholder: "Select Blockchain",
                                                    showSearch: true,
                                                    onChange: setSelectedBlockchain,
                                                    size: "large",
                                                    children: Object.keys(Chains).map((chain)=>/*#__PURE__*/ (0, $erTRO$jsx)("option", {
                                        ...
                                        /*#__PURE__*/ (0, $erTRO$jsxs)("div", {
                                            ...
                                            children: [
                                                ...
                                                /*#__PURE__*/ (0, $erTRO$jsx)((0, $erTRO$Select), {
                                                    ...
                                                    size: "large",
                                                    children: Tokens.filter((obj)=>(0, $535c395a1d197c28$export$c12d5eb7df2a872c)[selectedBlockchain].includes(obj.name)).map((obj)=>/*#__PURE__*/ (0, $erTRO$jsx)("option", {
                                                            value: obj.name,
                                                            children: obj.name
                                                        }, obj.name))
                                                })
...
var $79b4685724ad28b1$export$2e2bcd8739ae039 = $79b4685724ad28b1$var$CryptoPayment;





export {$79b4685724ad28b1$export$2e2bcd8739ae039 as CryptoPayment, $535c395a1d197c28$export$c3f32f9b7c2f46bb as Chains, $535c395a1d197c28$export$8e1e81ac145e31be as Tokens};
